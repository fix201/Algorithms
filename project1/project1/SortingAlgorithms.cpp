...
public:
	...
	SortingAlgorithms(string fileNames[], double r[]) {
		...
		for (unsigned int i = 0; i < sources[0].size() ; i++)
	...
	//print first 10
	void printFirst10(vector<double> totalSum) {
...
int main() {
	...
	do
	{
		...
		for (int counter = 0; counter < numOfSources; counter++)
		{
			...
			temp.printFirst10(Sum.getSorces(counter));
		...
		cout << "\nTotal time taken for Loop " << ++loop << " is: " << (clock() - current_time)/1000 << " seconds." << endl;
		current_time = clock();
		cout << "previous realibility: " << setprecision(10) << preRealibility << endl;
		cout << "current realibility: " << realibility << endl << endl << endl;
